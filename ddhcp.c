...
ATTR_NONNULL_ALL int ddhcp_block_init(ddhcp_config *config)
{
	...
	config->blocks = (struct ddhcp_block *)calloc(
		sizeof(struct ddhcp_block), config->number_of_blocks);
	...
	for (uint32_t index = 0; index < config->number_of_blocks; index++) {
		...
		addr_add(&config->prefix, &block->subnet,
			 (int)(index * config->block_size));
...
ATTR_NONNULL_ALL int ddhcp_check_packet(struct ddhcp_mcast_packet *packet,
					ddhcp_config *config)
{
	if (memcmp(&packet->prefix, &config->prefix, sizeof(struct in_addr)) !=
		    0 ||
...
ATTR_NONNULL_ALL void ddhcp_block_process(uint8_t *buffer, ssize_t len,
					  ...
					  ddhcp_config *config)
{
	...
	if (ret == 0) {
		...
		case DDHCP_MSG_UPDATECLAIM:
			statistics_record(config, STAT_MCAST_RECV_UPDATECLAIM,
					  1);
			   ...
			   ddhcp_config *config)
{
	...
	for (i = 0; i < packet->count; i++) {
		...
		if (blocks[block_index].state == DDHCP_OURS &&
			...
			memcpy(&blocks[block_index].node_id, &packet->node_id,
			       sizeof(ddhcp_node_id));

#if LOG_LEVEL_LIMIT >= LOG_DEBUG
			char ipv6_sender[INET6_ADDRSTRLEN];
			DEBUG("ddhcp_block_process_claims(...): Register block to %s\n",
			      inet_ntop(AF_INET6,
					&blocks[block_index].owner_address,
					ipv6_sender, INET6_ADDRSTRLEN));
#endif
		...
		if (blocks[tmp->block_index].state == DDHCP_OURS) {
			...
			if (NODE_ID_CMP(packet->node_id, config->node_id) > 0) {
				...
				blocks[tmp->block_index].state =
					DDHCP_TENTATIVE;
				blocks[tmp->block_index].timeout =
...
ATTR_NONNULL_ALL void ddhcp_dhcp_process(uint8_t *buffer, ssize_t len,
					 struct sockaddr_in6 sender,
					 ddhcp_config *config)
{
	struct ddhcp_mcast_packet packet;
					    ...
					    ddhcp_config *config)
{
	...
	ssize_t bytes_send = send_packet_direct(
		answer, &packet->sender->sin6_addr, DDHCP_SKT_SERVER(config));
					  ...
					  ddhcp_config *config)
{
	...
	dhcp_packet *packet =
		dhcp_packet_list_find(&config->dhcp_packet_cache,
				      request->renew_payload->xid,
