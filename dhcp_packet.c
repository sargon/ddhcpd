...
int ntoh_dhcp_packet(dhcp_packet *packet,char* buffer,int len) {
  ...
  memcpy(&tmp32,buffer+4,4); packet->xid   = ntohl(tmp32);
  memcpy(&tmp16,buffer+8,2); packet->secs  = ntohs(tmp16);
  memcpy(&tmp16,buffer+10,2); packet->flags = ntohs(tmp16);
...
int send_dhcp_packet(int socket, dhcp_packet *packet) {
  ...
  tmp32 = htonl(packet->xid); memcpy(buffer+4, &tmp32,4);
  tmp16 = htons(packet->secs); memcpy(buffer+8,&tmp16,2);
  tmp16 = htons(packet->flags); memcpy(buffer+10,&tmp16,2);
