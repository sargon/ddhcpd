...
ATTR_NONNULL_ALL void house_keeping(ddhcp_config *config)
{
	...
	int32_t leases_needed =
		(int32_t)config->spare_leases_needed - (int32_t)spare_leases;
...
ATTR_NONNULL_ALL int hdl_ddhcp_dhcp(epoll_data_t data, ddhcp_config *config)
{
	...
	while ((len = recvfrom(fd, buffer, 1500, 0, (struct sockaddr *)&sender,
		...
		DEBUG("Receive message from %s\n",
		      inet_ntop(AF_INET6, &in_addr, ipv6_sender,
				INET6_ADDRSTRLEN));
#endif
...
int main(int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, "C:c:i:St:dvVDhLb:B:N:o:s:H:n:")) !=
	       -1) {
		switch (c) {
		case 'i':
			interface = optarg;
	...
	config.sockets[SKT_MCAST] = epoll_data_new(
		interface, netsock_multicast_init, hdl_ddhcp_block, NULL);
	config.sockets[SKT_SERVER] = epoll_data_new(
		interface, netsock_server_init, hdl_ddhcp_dhcp, NULL);
	config.sockets[SKT_CONTROL] = epoll_data_new(
		config.control_path, netsock_control_init, hdl_ctrl_new, NULL);
	...
	epoll_add_fd(config.epoll_fd, config.sockets[SKT_SERVER],
		     EPOLLIN | EPOLLET, &config);
	epoll_add_fd(config.epoll_fd, config.sockets[SKT_CONTROL],
		     EPOLLIN | EPOLLET, &config);
	epoll_add_fd(config.epoll_fd, netlink, EPOLLIN | EPOLLET, &config);

	if (config.disable_dhcp == 0) {
		config.sockets[SKT_DHCP] = epoll_data_new(
			interface_client, netsock_dhcp_init, hdl_dhcp, NULL);
		epoll_add_fd(config.epoll_fd, config.sockets[SKT_DHCP],
			     EPOLLIN | EPOLLET, &config);
	}
	...
	do {
		...
		for (int i = 0; i < n; i++) {
			ddhcp_epoll_data *data =
				(ddhcp_epoll_data *)events[i].data.ptr;
